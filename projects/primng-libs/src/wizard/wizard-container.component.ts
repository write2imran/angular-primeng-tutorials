...
export class StepsComponent implements AfterContentInit, OnChanges {
    ...
    @ContentChildren(WizardChildComponent) steps: QueryList<WizardChildComponent>;

    ngAfterContentInit() {
        this.steps.toArray().forEach((step: WizardChildComponent, index: number) => {
            if (!step.styleClass) {
            ...
            this.items[index] = {
                ...
                command: (event: any) => {
                    // hide all steps
                    this.steps.toArray().forEach((s: WizardChildComponent) => s.active = false);
